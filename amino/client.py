...
class Client:
    ...
    def register(self, nickname: str, email: str, password: str, deviceId: str = device.device_id):
        ...
        **Returns**
        ...
        response = requests.post(f"{self.api}/g/s/auth/register", data=data, headers=custom_headers, proxies=self.proxies, verify=self.certificatePath)
        if response.status_code != 200: return exceptions.CheckException(json.loads(response.text))
    ...
    def verify(self, deviceId, email: str, code: str):
        """
        Verify an account.
    ...
    def request_verify_code(self, deviceId, email: str, resetPassword: bool = False):
        """
