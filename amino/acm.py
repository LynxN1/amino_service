...
class ACM(client.Client):
    ...
    def create_community(self, name: str, tagline: str, icon: BinaryIO, themeColor: str, joinType: int = 0,
        ...
        if response.status_code != 200:
            ...
            if response["api:statuscode"] == 2800:
                raise exceptions.AccountAlreadyRestored
            else:
                return response
    ...
    def delete_community(self, email: str, password: str, verificationCode: str):
        ...
        response = requests.post(f"{self.api}/g/s-x{self.comId}/community/delete-request",
                                 headers=headers.Headers(data=data).headers, data=data)
        if response.status_code != 200:
            response = json.loads(response.text)
            ...
            if response["api:statuscode"] == 833:
                raise exceptions.CommunityDeleted
            else:
                return response
    ...
    def list_communities(self, start: int = 0, size: int = 25):
        response = requests.get(f"{self.api}/g/s/community/managed?start={start}&size={size}",
    ...
    def edit_modules(self, module: str, value):
        ...
        if self.comId is None: raise exceptions.CommunityNeeded
        response = requests.post(f"{self.api}/x{self.comId}/s/community/configuration",
                                 headers=headers.Headers(data=data).headers, data=data)
        return json.loads(response.text)

    def get_categories(self, start: int = 0, size: int = 25):
        if self.comId is None: raise exceptions.CommunityNeeded
        response = requests.get(f"{self.api}/x{self.comId}/s/blog-category?start={start}&size={size}",
                                headers=headers.Headers().headers)
        return json.loads(response.text)
    ...
    def upload_themepack_raw(self, file: BinaryIO):
        if self.comId is None: raise exceptions.CommunityNeeded
        response = requests.post(f"{self.api}/x{self.comId}/s/media/upload/target/community-theme-pack",
                                 data=file.read(), headers=headers.Headers(data=file.read()).headers)
        if response.status_code == 200:
    ...
    def upload_theme(self, themePackUrl: str):
        ...
        if self.comId is None: raise exceptions.CommunityNeeded
        response = requests.post(f"{self.api}/x{self.comId}/s/community/settings", data=data,
